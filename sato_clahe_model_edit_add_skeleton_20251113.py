...
#MODEL_PATH = r"C:\Users\dromii\Downloads\900_hc_unetpp_final_epoch_70.pth"
MODEL_PATH = r"C:\Users\dromii\Downloads\940_hc_unetpp_final_epoch_90.pth"
...
class App:
    def apply_skeleton(self, mask_u8, p):
        """
        mask_u8: 최종 0/255 마스크
        p['use_skel']: 스켈레톤 사용 여부
        p['skel_thick']: 스켈레톤 후 두께 (픽셀)
        """
        ...
        # 1) 스켈레톤화 (1픽셀 두께 bone만 남김)
        skel_bool = skeletonize(mask_u8 > 0)
        skel_u8 = (skel_bool.astype(np.uint8) * 255)

        # 2) 두께 확대 (원하면 1픽셀 그대로 두고 싶으면 조건 걸어도 됨)
        t = max(1, p.get("skel_thick", 1))
        if t > 1:
            k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2*t+1, 2*t+1))
            skel_u8 = cv2.dilate(skel_u8, k, iterations=1)
    ...
    def save_final(self):
        ...
        final = self.apply_skeleton(final, p)
